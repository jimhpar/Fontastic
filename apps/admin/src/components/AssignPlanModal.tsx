        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <select
              ...
              style={{ width: '100%' }}
            >
              <option value="">Revoke Plan / Set Inactive</option>
              {plans.map(p => (
                <option key={p._id} value={p._id}>
                  {p.name} — {p.priceBDT} TK/mo ({p.searchesPerWeek ? `${p.searchesPerWeek} searches/wk` : 'Unlimited searches'})
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <span style={{ fontSize: 12, color: '#94a3b8', marginTop: 4, display: 'block' }}>
              Standard month = 30 days, 1 year = 365 days.
            </span>
          </div>
